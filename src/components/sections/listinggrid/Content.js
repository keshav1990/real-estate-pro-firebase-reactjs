...
class Content extends Component {
    constructor() {
        ...
        this.state = {
            ...
            currentListing: null,
			currentIndex: -1,
            filters:{type:'Any Type',beds:'Any amount',bathrooms:'Any amount',status:'Any Status'}
        };
        ...
        this.doFilterData = this.doFilterData.bind(this);
    }

    onChangeBathrooms(e) {
        

        const bathroomsVal = e.target.value;
        console.log(bathroomsVal);
    ...
    doFilterData(e){
        ...
        let orignal = this.state.orignalitems;
        let filterState = {type:'Any Type',beds:'Any amount',bathrooms:'Any amount',status:'Any Status'}
        var items = orignal.filter(item => {
            //console.log(item  
            return Object.keys(filterState).every(function(filter, index, array) {
                console.log(filters[filter] );

                if(filterState[filter]== filters[filter] ){
                    return true;
                 }else{
    ...
    handleClick(event) {
	 ...
	 componentDidMount() {
    ListingsDataService.getAll().on("value", this.onDataChange);
  }

  componentWillUnmount() {
    ListingsDataService.getAll().off("value", this.onDataChange);
  }
  ...
  onDataChange= (listingsData)=>{
    ...
    this.setState({
      items: listing,
      orignalitems:listing
    });
  }
  
  
  setActiveListing(listingval, index) {
    this.setState({
      currentListing: listingval,
      currentIndex: index,
    });
  }
    ...
    render() {
        ...
        const renderitems = currentitems.map((item, i) => {
            return <div key={item.key} className="col-md-6" onClick={() => this.setActiveListing(item, item.key)}>
                <div className="listing" >
                    ...
                    <div className="listing-body">
                        ...
                        <span className="listing-price">{item.monthlyprice}$ <span>/month</span> </span>
                ...
                <div className="container">
                    <div className="row">
                        ...
                        <div className="col-lg-4">
                            <Sidebar  onChangeBathrooms={this.onChangeBathrooms} onChangeBed={this.onChangeBed} 
                            onChangeStatus={this.onChangeStatus} doFilterData={this.doFilterData} onChangeType={this.onChangeType}
                            />
                        </div>
                        {/* Sidebar End */}
                        {/* Posts Start */}
